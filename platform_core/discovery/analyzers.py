...
class LanguageAnalyzer(BaseAnalyzer):
    ...
    CONFIG_EXTENSIONS = {".yaml", ".yml", ".json", ".toml", ".xml", ".ini", ".cfg", ".conf"}
...
class FrameworkAnalyzer(BaseAnalyzer):
    ...
    KNOWN_FRAMEWORKS: list[dict[str, Any]] = [
        {"name": "FastAPI", "signals": ["fastapi"], "category": "web", "file": "requirements.txt"},
        {"name": "Django", "signals": ["django"], "category": "web", "file": "requirements.txt"},
        {"name": "Flask", "signals": ["flask"], "category": "web", "file": "requirements.txt"},
        {"name": "Express", "signals": ["express"], "category": "web", "file": "package.json"},
        {"name": "Next.js", "signals": ["next"], "category": "web", "file": "package.json"},
        {"name": "React", "signals": ["react"], "category": "frontend", "file": "package.json"},
        {"name": "Vue", "signals": ["vue"], "category": "frontend", "file": "package.json"},
        {"name": "Angular", "signals": ["@angular/core"], "category": "frontend", "file": "package.json"},
        {"name": "Svelte", "signals": ["svelte"], "category": "frontend", "file": "package.json"},
        {"name": "pytest", "signals": ["pytest"], "category": "test", "file": "requirements.txt"},
        {"name": "Jest", "signals": ["jest"], "category": "test", "file": "package.json"},
        {"name": "SQLAlchemy", "signals": ["sqlalchemy"], "category": "orm", "file": "requirements.txt"},
        {"name": "Pydantic", "signals": ["pydantic"], "category": "validation", "file": "requirements.txt"},
        {"name": "Celery", "signals": ["celery"], "category": "task_queue", "file": "requirements.txt"},
        {"name": "Redis", "signals": ["redis"], "category": "cache", "file": "requirements.txt"},
        {"name": "TensorFlow", "signals": ["tensorflow"], "category": "ml", "file": "requirements.txt"},
        {"name": "PyTorch", "signals": ["torch"], "category": "ml", "file": "requirements.txt"},
    ]
...
class ArchitectureAnalyzer(BaseAnalyzer):
    ...
    PATTERNS: list[dict[str, Any]] = [
        {"name": "microservices", "signals": ["services/", "docker-compose.yml"], "min_matches": 1},
        ...
        {"name": "serverless", "signals": ["handler.py", "lambda", "serverless.yml"], "min_matches": 1},
        {"name": "plugin_based", "signals": ["plugins/", "hooks/"], "min_matches": 1},
        {"name": "layered", "signals": ["models/", "views/", "controllers/"], "min_matches": 2},
    ]
...
class DocumentationAnalyzer(BaseAnalyzer):
    ...
    def analyze(self, root_path: str, files: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if not has_readme:
            findings.append({"type": "missing_readme", "severity": "high", "message": "No README file found"})
        if not has_license:
            findings.append({"type": "missing_license", "severity": "medium", "message": "No LICENSE file found"})
        if not has_changelog:
            findings.append({"type": "missing_changelog", "severity": "low", "message": "No CHANGELOG.md found"})
...
class TestingAnalyzer(BaseAnalyzer):
    ...
    TEST_PATTERNS = [
        ...
        "*.test.tsx", "*.test.jsx", "*_test.go", "*_test.rs",
        "*.spec.ts", "*.spec.js",
    ]
    TEST_CONFIG_FILES = [
        "pytest.ini", "setup.cfg", "pyproject.toml",
        "jest.config.js", "jest.config.ts", ".jest.config",
    ...
    def analyze(self, root_path: str, files: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        has_coverage = any(
            any(c in f["path"] for c in self.COVERAGE_FILES)
            for f in files
        )
        ...
        findings: list[dict[str, Any]] = []
        if test_count == 0:
            findings.append({"type": "no_tests", "severity": "high", "message": "No test files found"})
        if not has_test_config:
            findings.append({"type": "no_test_config", "severity": "low", "message": "No test configuration found"})
    ...
    def _is_test_file(self, name: str) -> bool:
        ...
        test_suffixes = ("_test.py", "_test.ts", "_test.js", "_test.go", "_test.rs",
...
def run_all_analyzers(
    root_path: str, files: list[dict[str, Any]]
) -> AnalysisResult:
    ...
    for analyzer in analyzers:
        ...
        for f in analysis.get("findings", []):
            findings.append(Finding(
                category=FindingCategory(analyzer.name if analyzer.name != "ci_cd" else "ci_cd"),
                type=f.get("type", ""),
