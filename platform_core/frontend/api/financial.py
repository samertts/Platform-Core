...
@dataclass
class InsuranceClient(BaseEntityClient[Any]):
    ...
    def verify(self, insurance_id: str) -> dict[str, Any]:
        return self.client.get(f"{self.endpoint}/{insurance_id}/verify")

    def get_active(self, patient_id: str) -> dict[str, Any]:
        return self.client.get(f"{self.endpoint}/patient/{patient_id}/active")


@dataclass
class ConsentClient(BaseEntityClient[Any]):
    ...
    def grant(self, consent_data: dict[str, Any]) -> dict[str, Any]:
    ...
    def check(self, patient_id: str, consent_type: str, target: str) -> dict[str, Any]:
...
@dataclass
class InvoiceClient(BaseEntityClient[Any]):
    ...
    def get_outstanding(self, patient_id: str) -> dict[str, Any]:
        return self.client.get(f"{self.endpoint}/patient/{patient_id}/outstanding")

    def mark_paid(self, invoice_id: str, amount: float) -> dict[str, Any]:
        return self.client.put(f"{self.endpoint}/{invoice_id}/paid", data={"amount": amount})


@dataclass
class PaymentClient(BaseEntityClient[Any]):
    ...
    def get_by_invoice(self, invoice_id: str) -> dict[str, Any]:
...
@dataclass
class ClaimClient(BaseEntityClient[Any]):
    ...
    def get_by_insurance(self, insurance_id: str) -> dict[str, Any]:
        return self.client.get(f"{self.endpoint}/insurance/{insurance_id}")

    def submit(self, claim_data: dict[str, Any]) -> dict[str, Any]:
        return self.client.post(f"{self.endpoint}/submit", data=claim_data)

    def approve(self, claim_id: str, approved_amount: float) -> dict[str, Any]:
        return self.client.put(
            f"{self.endpoint}/{claim_id}/approve",
            data={"approved_amount": approved_amount},
        )

    def reject(self, claim_id: str, reason: str) -> dict[str, Any]:
