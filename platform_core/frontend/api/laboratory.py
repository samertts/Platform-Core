...
@dataclass
class SpecimenClient(BaseEntityClient[Any]):
    ...
    def collect(self, specimen_id: str, collector_id: str) -> dict[str, Any]:
        return self.client.put(
            f"{self.endpoint}/{specimen_id}/collect",
            data={"collector_id": collector_id},
        )

    def receive(self, specimen_id: str) -> dict[str, Any]:
        return self.client.put(f"{self.endpoint}/{specimen_id}/receive")

    def reject(self, specimen_id: str, reason: str) -> dict[str, Any]:
        return self.client.put(f"{self.endpoint}/{specimen_id}/reject", data={"reason": reason})


@dataclass
class LaboratoryOrderClient(BaseEntityClient[Any]):
...
@dataclass
class LaboratoryResultClient(BaseEntityClient[Any]):
    ...
    def get_by_order(self, order_id: str) -> dict[str, Any]:
        return self.client.get(f"{self.endpoint}/order/{order_id}")

    def get_by_patient(self, patient_id: str) -> dict[str, Any]:
        return self.client.get(f"{self.endpoint}/patient/{patient_id}")

    def validate(self, result_id: str, validator_id: str) -> dict[str, Any]:
        return self.client.put(
            f"{self.endpoint}/{result_id}/validate", data={"validator_id": validator_id}
        )

    def get_critical_results(self, facility_id: str) -> dict[str, Any]:
        return self.client.get(f"{self.endpoint}/critical", params={"facility_id": facility_id})


@dataclass
class AnalyzerClient(BaseEntityClient[Any]):
    ...
    def get_status(self, analyzer_id: str) -> dict[str, Any]:
        return self.client.get(f"{self.endpoint}/{analyzer_id}/status")

    def calibrate(self, analyzer_id: str, calibration_data: dict[str, Any]) -> dict[str, Any]:
