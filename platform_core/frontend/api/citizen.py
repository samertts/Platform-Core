...
@dataclass
class CitizenClient(BaseEntityClient[Any]):
    ...
    def get_by_national_id(self, national_id: str) -> dict[str, Any]:
        return self.client.get(f"{self.endpoint}/national-id/{national_id}")

    def verify_identity(self, citizen_id: str, verification_data: dict[str, Any]) -> dict[str, Any]:
        return self.client.post(f"{self.endpoint}/{citizen_id}/verify", data=verification_data)

    def suspend(self, citizen_id: str, reason: str) -> dict[str, Any]:
        return self.client.put(f"{self.endpoint}/{citizen_id}/suspend", data={"reason": reason})

    def reactivate(self, citizen_id: str) -> dict[str, Any]:
        return self.client.put(f"{self.endpoint}/{citizen_id}/reactivate")


@dataclass
class DigitalCredentialClient(BaseEntityClient[Any]):
    ...
    def revoke(self, credential_id: str, reason: str) -> dict[str, Any]:
        return self.client.put(f"{self.endpoint}/{credential_id}/revoke", data={"reason": reason})

    def verify(self, credential_id: str) -> dict[str, Any]:
