...
@dataclass
class DiagnosisClient(BaseEntityClient[Any]):
    ...
    def confirm(self, diagnosis_id: str) -> dict[str, Any]:
        return self.client.put(f"{self.endpoint}/{diagnosis_id}/confirm")

    def resolve(self, diagnosis_id: str, resolution_date: str) -> dict[str, Any]:
...
@dataclass
class ProcedureClient(BaseEntityClient[Any]):
    ...
    def complete(self, procedure_id: str, notes: str) -> dict[str, Any]:
        return self.client.put(f"{self.endpoint}/{procedure_id}/complete", data={"notes": notes})


@dataclass
class ObservationClient(BaseEntityClient[Any]):
    ...
    def record(self, observation_data: dict[str, Any]) -> dict[str, Any]:
        return self.client.post(f"{self.endpoint}/record", data=observation_data)


@dataclass
class VitalSignsClient(BaseEntityClient[Any]):
    ...
    def record(self, vitals_data: dict[str, Any]) -> dict[str, Any]:
        return self.client.post(f"{self.endpoint}/record", data=vitals_data)

    def get_trends(self, patient_id: str, vital_type: str, days: int = 30) -> dict[str, Any]:
