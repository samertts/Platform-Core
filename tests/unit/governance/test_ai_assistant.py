...
class TestAIGovernanceAssistant:
    ...
    def test_review_dependencies(self) -> None:
        ai = AIGovernanceAssistant()
        result = ai.review_dependencies("repo", {"total_dependencies": 10, "vulnerable_dependencies": 2})
    ...
    def test_review_api(self) -> None:
        ai = AIGovernanceAssistant()
        result = ai.review_api("repo", {
            "has_health_endpoint": True, "has_api_docs": True, "api_versioned": True,
        })
        assert result["score"] == 1.0
    ...
    def test_generate_remediation_plan(self) -> None:
        ai = AIGovernanceAssistant()
        findings = [
            Finding(severity=FindingSeverity.CRITICAL, title="Critical", recommendation="Fix now"),
