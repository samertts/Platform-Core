...
def _make_record(tmp_path: Path, name: str = "pkg", version: str = "1.0.0") -> InstallRecord:
...
class TestRollbackEngine:
    ...
    def test_delete_nonexistent_snapshot(self, tmp_path: Path) -> None:
        engine = RollbackEngine(storage_root=str(tmp_path / "rollback"))
    ...
    def test_add_operation(self, tmp_path: Path) -> None:
        engine = RollbackEngine(storage_root=str(tmp_path / "rollback"))
        txn = engine.begin_transaction()
        engine.add_operation(str(txn.id), "install", "pkg")
        assert len(txn.operations) == 1

    def test_add_operation_nonexistent(self, tmp_path: Path) -> None:
        engine = RollbackEngine(storage_root=str(tmp_path / "rollback"))
        with pytest.raises(RollbackError):
            engine.add_operation("nonexistent", "install", "pkg")

    def test_commit_transaction(self, tmp_path: Path) -> None:
        engine = RollbackEngine(storage_root=str(tmp_path / "rollback"))
        txn = engine.begin_transaction()
        engine.add_operation(str(txn.id), "install", "pkg")
    ...
    def test_rollback_transaction(self, tmp_path: Path) -> None:
        engine = RollbackEngine(storage_root=str(tmp_path / "rollback"))
        txn = engine.begin_transaction()
        engine.add_operation(str(txn.id), "install", "pkg")
    ...
    def test_rollback_transaction_nonexistent(self, tmp_path: Path) -> None:
        engine = RollbackEngine(storage_root=str(tmp_path / "rollback"))
        with pytest.raises(RollbackError):
            engine.rollback_transaction("nonexistent")

    def test_auto_rollback(self, tmp_path: Path) -> None:
        engine = RollbackEngine(storage_root=str(tmp_path / "rollback"))
        txn = engine.begin_transaction()
        engine.add_operation(str(txn.id), "install", "pkg")
    ...
    def test_verify_consistency(self, tmp_path: Path) -> None:
        engine = RollbackEngine(storage_root=str(tmp_path / "rollback"))
        packages = {
            "pkg": InstallRecord(
                package_name="pkg",
    ...
    def test_verify_consistency_empty(self, tmp_path: Path) -> None:
        engine = RollbackEngine(storage_root=str(tmp_path / "rollback"))
    ...
    def test_get_transaction_nonexistent(self, tmp_path: Path) -> None:
        engine = RollbackEngine(storage_root=str(tmp_path / "rollback"))
