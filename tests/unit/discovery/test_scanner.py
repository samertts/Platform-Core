...
class TestScanner:
    ...
    def test_scan_file_not_dir(self, tmp_path: Path) -> None:
        f = tmp_path / "file.txt"
        f.write_text("hello")
        scanner = Scanner()
    ...
    def test_scan_excludes_large_files(self, tmp_path: Path) -> None:
    ...
    def test_get_file_tree(self, tmp_path: Path) -> None:
        (tmp_path / "main.py").write_text("x = 1")
        sub = tmp_path / "src"
        sub.mkdir()
    ...
    def test_get_repository_metadata(self, tmp_path: Path) -> None:
        (tmp_path / "README.md").write_text("# Test")
    ...
    def test_scan_incremental(self, tmp_path: Path) -> None:
        (tmp_path / "main.py").write_text("x = 1")
        scanner = Scanner()
    ...
    def test_scan_duration(self, tmp_path: Path) -> None:
        (tmp_path / "main.py").write_text("x = 1")
        scanner = Scanner()
        result = scanner.scan(str(tmp_path))
