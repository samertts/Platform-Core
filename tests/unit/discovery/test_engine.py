...
class TestDiscoveryEngine:
    ...
    def test_discover_with_files(self, tmp_path: Path) -> None:
    ...
    def test_discover_stores_result(self, tmp_path: Path) -> None:
    ...
    def test_get_ecosystem_report(self, tmp_path: Path) -> None:
        (tmp_path / "main.py").write_text("x = 1")
        engine = DiscoveryEngine()
        engine.discover(str(tmp_path), repository_name="r1")
    ...
    def test_get_repository_report(self, tmp_path: Path) -> None:
        (tmp_path / "main.py").write_text("x = 1")
        engine = DiscoveryEngine()
        engine.discover(str(tmp_path), repository_name="r1")
    ...
    def test_discover_incremental(self, tmp_path: Path) -> None:
        (tmp_path / "main.py").write_text("x = 1")
        engine = DiscoveryEngine()
